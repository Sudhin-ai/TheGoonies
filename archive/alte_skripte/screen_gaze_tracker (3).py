...
def detect_from_diff(frame, reference):
    ...
    # Rauschen entfernen + Ring schließen:
    # Der Tobii-Kreis ist halbtransparent, d.h. nur sein Rand ändert sich
    # im Differenzbild - die Mitte bleibt leer (Ring statt Scheibe).
    # Ein großer CLOSE-Kernel füllt das Innere des Rings auf, damit der
    # Schwerpunkt (Centroid) wirklich in der Mitte des Kreises liegt.
    ...
    # Schwerpunkt (Centroid) der gefüllten Fläche = echter Mittelpunkt des Kreises.
    # Robuster als minEnclosingCircle, das bei einem Ring zu weit außen landet.
    M = cv2.moments(best)
    if M["m00"] == 0:
        return None
    cx = M["m10"] / M["m00"]
    cy = M["m01"] / M["m00"]
    # Radius aus der Fläche zurückrechnen (Fläche = π·r²)
